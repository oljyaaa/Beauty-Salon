...
export default function CreatorDashboard({ user }: { user: any }) {
  ...
  try {
    ...
    const res = await fetch(API_URL, { method: 'POST', body: f });
    const textResponse = await res.text(); // Спочатку читаємо як текст
    ...
    try {
      ...
      if (json.status === 'success') {
          ...
          // Тепер, якщо є помилка в базі даних, ми побачимо її тут!
          Alert.alert("Помилка на сервері", json.message);
      }
    } catch (parseError) {
      console.error("Неочікувана відповідь від сервера:", textResponse);
      Alert.alert("Критична помилка", "Сервер повернув невірні дані. Глянь консоль.");
    }
  } catch (e) { 
    Alert.alert("Помилка", "Дійсно проблема з мережею або сервер ліг."); 
  }
};
  ...
  <View style={{flex: 1, backgroundColor: '#FFF', padding: 20}}>
      ...
      <TextInput style={styles.input} value={pricingForm.name} onChangeText={t => setPricingForm({...pricingForm, name: t})} placeholder="Назва послуги" />
      
      {/* Текстова ціна (для відображення) */}
      <TextInput style={styles.input} value={pricingForm.price_string} onChangeText={t => setPricingForm({...pricingForm, price_string: t})} placeholder="Ціна (текстом, напр. 'від 500 грн')" />
      
      {/* Числова ціна (для КОШИКА) */}
      <TextInput style={styles.input} value={pricingForm.price_numeric} onChangeText={t => setPricingForm({...pricingForm, price_numeric: t})} placeholder="Ціна для кошика (лише число, напр. '500')" keyboardType="numeric" />
