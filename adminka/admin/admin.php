...
$tabs = [
    ...
    'Elos-епіляція',
    'Доглядові процедури',
    'Доглядові процедури — Ольга',
    'Доглядові процедури — Катерина',
    'Доглядові процедури — Євгенія',
    'Доглядові процедури — Юля',
    'archive',
    'call',
    'comments'
];
...
// --- CREATE RECORD FROM "CALL BACK" APPOINTMENT ---
if (isset($_POST['add_from_appointment'])) {
    ...
    if (!can_access($category)) {
        echo "<script>alert('⛔️ У вас немає прав додавати записи в цю категорію!'); window.history.back();</script>";
        exit;
    } else {
        ...
        if ($stmt_check->num_rows > 0) {
            $stmt_check->close();
            echo "<script>alert('⛔️ Цей час уже зайнятий!'); window.history.back();</script>";
            exit;
        }
        $stmt_check->close();

        $stmt_insert = $conn->prepare("
            INSERT INTO records (client_name, phone, category, service_name, record_date, record_time, note)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt_insert->bind_param("sssssss", $client_name, $phone, $category, $service_name, $record_date, $record_time, $note);
        $stmt_insert->execute();
        $stmt_insert->close();

        $stmt_del = $conn->prepare("DELETE FROM appointments WHERE id = ?");
        $stmt_del->bind_param("i", $appointment_id);
        $stmt_del->execute();
        $stmt_del->close();

        header("Location: " . $_SERVER['PHP_SELF'] . "?tab=all");
        exit();
    }
}

// --- ADD RECORD (MAIN MODAL) ---
...
<head>
    ...
    <style>
        .appointment-to-record-form {
            margin-top: 12px;
            padding-top: 10px;
            border-top: 1px solid rgba(0,0,0,0.06);
        }
        .recall-form-row { margin-bottom: 8px; }
        .recall-form-row label {
            display:block;
            font-size:13px;
            margin-bottom:3px;
        }
        .recall-form-row input,
        .recall-form-row select,
        .recall-form-row textarea {
            width:100%;
            font-size:14px;
            padding:6px 8px;
            border-radius:8px;
        ...
        .recall-form-row-inline { display:flex; gap:8px; }
        .recall-form-row-inline > div { flex:1; }
...
<div class="sidebar">
    <div>
        ...
        <div class="menu">
            ...
            <?php endif; ?>
            <?php if (can_access('Доглядові процедури — Ольга')): ?>
            <button id="btn-Доглядові процедури — Ольга" onclick="showSection('Доглядові процедури — Ольга')">
                    <i class="fas fa-pump-soap"></i> Догляд — Ольга
            </button>
            <?php endif; ?>

            <?php if (can_access('Доглядові процедури — Катерина')): ?>
            <button id="btn-Доглядові процедури — Катерина" onclick="showSection('Доглядові процедури — Катерина')">
                <i class="fas fa-pump-soap"></i> Догляд — Катерина
            </button>
            <?php endif; ?>

            <?php if (can_access('Доглядові процедури — Євгенія')): ?>
            <button id="btn-Доглядові процедури — Євгенія" onclick="showSection('Доглядові процедури — Євгенія')">
                <i class="fas fa-pump-soap"></i> Догляд — Євгенія
            </button>
            <?php endif; ?>
            <?php if (can_access('Доглядові процедури — Юля')): ?>
            <button id="btn-Доглядові процедури — Юля" onclick="showSection('Доглядові процедури — Юля')">
                <i class="fas fa-pump-soap"></i> Догляд — Юля
            </button>
            <?php endif; ?>
    ...
    <div id="callSection" style="display:none;">
        ...
        <?php if($appointments && $appointments->num_rows > 0): $appointments->data_seek(0); while($row = $appointments->fetch_assoc()): ?>
            <div class="card" id="appointment-<?= $row['id'] ?>">
                <button class="action-btn danger" onclick="deleteAppointment(<?= $row['id'] ?>)" title="Закрити запит">
                    <i class="fas fa-times"></i>
                </button>

                <h3><?= htmlspecialchars($row['name']) ?></h3>
                <p><b>Телефон:</b> <?= htmlspecialchars($row['phone']) ?></p>
                <p><b>Послуга (з форми сайту):</b> <?= htmlspecialchars($row['service']) ?></p>
                ...
                <div style="margin-top:10px;">
                    <button type="button"
                            class="btn-primary-small open-appointment-modal"
                            data-id="<?= $row['id'] ?>"
...
<!-- MAIN "NEW RECORD" MODAL -->
<div id="recordModal" class="modal-overlay">
    <div class="modal-box" style="position:relative;">
        <button type="button" class="modal-close-btn" id="closeRecordModal">&times;</button>
...
<div id="editModal" class="modal-overlay">
    <div class="modal-box" style="position:relative;">
        <button type="button" class="modal-close-btn" id="closeEditModal">&times;</button>
...
<div id="appointmentModal" class="modal-overlay">
    <div class="modal-box" style="position:relative;">
        <button type="button" class="modal-close-btn" id="closeAppointmentModal">&times;</button>
        <h2>Новий запис з запиту</h2>
        <form id="appointmentForm" class="appointment-to-record-form" method="post">
            <input type="hidden" name="appointment_id" id="appointment_id">
            <input type="hidden" name="client_name" id="appointment_client_name">
            <input type="hidden" name="phone" id="appointment_phone">

            <div class="recall-form-row">
                <label>Тип процедури</label>
                <select name="category" id="appointment_category" required>
                    ...
                    <option value="Доглядові процедури — Ольга">Догляд — Ольга</option>
                    <option value="Доглядові процедури — Катерина">Догляд — Катерина</option>
                    <option value="Доглядові процедури — Євгенія">Догляд — Євгенія</option>
                </select>
            </div>

            <div class="recall-form-row">
                <label>Назва послуги</label>
                <input type="text" name="service_name" id="appointment_service_name"
                       placeholder="Наприклад: Лімфодренажний масаж" required>
            </div>

            <div class="recall-form-row">
                <label>Опис / примітка</label>
                <textarea name="note" id="appointment_note" rows="3"
                          placeholder="Коротко про побажання клієнта..."></textarea>
            </div>

            <div class="recall-form-row recall-form-row-inline">
                <div>
                    <label>Дата</label>
                    <input type="date" name="record_date" id="appointment_date" required>
                </div>
                <div>
                    <label>Час</label>
                    <select name="record_time" id="appointment_time" required>
                        <option value="">Час</option>
                        <option value="09:30">09:30</option><option value="10:30">10:30</option>
                        <option value="11:30">11:30</option><option value="12:30">12:30</option>
                        <option value="13:30">13:30</option><option value="14:30">14:30</option>
                        <option value="15:30">15:30</option><option value="16:30">16:30</option>
                        <option value="17:30">17:30</option><option value="18:30">18:30</option>
                    </select>
                </div>
            </div>

            <div style="margin-top:10px; display:flex; gap:8px; justify-content:flex-end;">
                <button type="submit" name="add_from_appointment" class="btn-primary-small">
                    Записати
                </button>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    const servicesData = {
        ...
        'Доглядові процедури - Ольга': ['Чистка обличчя', 'Пілінг', 'Карбоксітерапія', 'Маска для обличчя'],
        'Доглядові процедури - Катерина': ['Чистка обличчя', 'Пілінг', 'Карбоксітерапія', 'Маска для обличчя'],
        'Доглядові процедури - Євгенія': ['Чистка обличчя', 'Пілінг', 'Карбоксітерапія', 'Маска для обличчя'],
    };
    ...
    const appointmentClientInput = document.getElementById('appointment_client_name');
    const appointmentPhoneInput = document.getElementById('appointment_phone');
    ...
    if (closeRecordModalBtn) closeRecordModalBtn.addEventListener('click', closeRecordModal);
    ...
    document.querySelectorAll('.open-appointment-modal').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            appointmentClientInput.value  = btn.dataset.name || '';
            appointmentPhoneInput.value   = btn.dataset.phone || '';
            appointmentNote.value         = btn.dataset.message || '';
...
function showSection(sec) {
    ...
    const categoryIcons = {'Масаж': 'fa-spa', 'Elos-епіляція': 'fa-star', 'Доглядові процедури': 'fa-pump-soap', 'Доглядові процедури — Ольга': 'fa-pump-soap', 'Доглядові процедури — Катерина': 'fa-pump-soap','Доглядові процедури — Євгенія': 'fa-pump-soap'};
...
function deleteAppointment(id) {
    if (!confirm("Видалити / закрити цей запит на дзвінок?")) return;
    fetch("", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
